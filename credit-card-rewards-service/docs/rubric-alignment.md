...
|---|---|---|
| SQL/data design | `credit_card_service/db.py`: 12+ normalized tables, foreign keys, CHECK constraints, partial UNIQUE indexes, WAL mode | `make test` & `python3 scripts/demo_acceptance.py` |
...
| Performance & Load | `scripts/benchmark.py`: concurrent throughput and percentile latency profiling | Concurrency=10, 200 reqs: p95 latency < 20ms, >650 req/sec |
...
| Frontend UI | Built-in responsive HTML5/CSS3 dashboard at `/` and `/ui` with Card Explorer, Comparison Matrix, and Review Portal | Zero-dependency SPA served at `http://localhost:8080/` |
...
| Testing | Automated unit and integration test suites (`test_service.py`, `test_v2_api.py`) | 24 tests passing with zero ResourceWarnings (`make test`) |
